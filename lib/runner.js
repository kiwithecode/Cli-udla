...
const { MCP_CONFIG, PKG_DIR, PKG_NAME } = require('./config');
...
function generatePlan(cfg, target, extra) {
  ...
  return new Promise((resolve) => {
    let label = 'Analizando el servicio y armando los casos de prueba';
    if (cfg.playwright) label = 'Explorando la web y armando el plan de pruebas';
    else if (cfg.tipo === 'movil') label = 'Explorando la app y armando los flujos de prueba';
    const sp = spinner.start(label);
  ...
  return new Promise((resolve) => {
    ...
    const finish = (status) => {
      ...
      if (status === 0) {
        ...
        // Para comandos con showOutput, mostramos el resumen final del agente
        // (p. ej. la tabla de casos ✅/❌ de las pruebas de API).
        if (cfg.showOutput && buf.trim()) log('\n' + buf.trim() + '\n');
...
// udla-qa update → trae la última versión y refresca el navegador.
// Dos modos según cómo se instaló:
//  - Instalación global (npm i -g udla-qa): reinstala desde npm (su postinstall
//    instala el navegador). No requiere haber clonado el repo.
//  - Clon git (desarrollo): git pull --ff-only + verificación del navegador.
async function update() {
  ...
  if (isGit.status !== 0) {
    ...
    log(s.kv('Origen', 'npm · ' + spec));
    const code = await spawnSpin(
      'npm', ['install', '-g', spec],
      'Reinstalando desde npm (incluye navegador de Playwright)',
      'UDLA-QA actualizado desde npm',
      'No se pudo reinstalar desde npm'
    );
    if (code === 0) log('\n' + s.ok('Listo. Reiniciá udla-qa para usar la nueva versión.'));
    else log(s.info('Probá manualmente:  npm install -g ' + spec));
    process.exit(code === 0 ? 0 : (code || 1));
  }

  // Modo desarrollador: clon git.
