...
const { MCP_CONFIG, PKG_DIR, REPO_SPEC } = require('./config');
...
function generatePlan(cfg, target, extra) {
  ...
  return new Promise((resolve) => {
    const label = cfg.playwright
      ? 'Explorando la web y armando el plan de pruebas'
      : 'Analizando el servicio y armando los casos de prueba';
...
//  - Instalación global (npm i -g github:…): reinstala desde GitHub (su postinstall
...
async function update() {
  ...
  if (isGit.status !== 0) {
    log(s.kv('Origen', REPO_SPEC));
    const code = await spawnSpin(
      'npm', ['install', '-g', REPO_SPEC],
      'Reinstalando desde GitHub (incluye navegador de Playwright)',
      'UDLA-QA actualizado desde GitHub',
      'No se pudo reinstalar desde GitHub'
    );
    if (code === 0) log('\n' + s.ok('Listo. Reiniciá udla-qa para usar la nueva versión.'));
    else log(s.info('Probá manualmente:  npm install -g ' + REPO_SPEC));
