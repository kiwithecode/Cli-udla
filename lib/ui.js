...
function help() {
  ...
  log(cmdRow('movil', '[app]', 'App móvil (Flutter/nativa · Android/iOS) con Maestro: flujos + run + ✅/❌'));
...
async function menu() {
  ...
  log(menuRow(6, 'Probar una app móvil ' + c.gray + '(Flutter/nativa · Android/iOS · Maestro)' + c.reset));
  log(menuRow(7, 'Actualizar el agente'));
  log(menuRow(8, 'Salir') + '\n');

  const KEYS = { 1: 'qa', 2: 'seguridad', 3: 'carga', 4: 'auditar', 5: 'servicio', 6: 'movil' };
  const ASKS = {
    1: '  URL a probar: ',
    2: '  URL a probar: ',
    3: '  URL de la API a probar: ',
    4: '  Ruta del repo a auditar (Enter = directorio actual): ',
    5: '  URL del endpoint (Enter si vas a usar un archivo con varios endpoints): ',
    6: '  App: package/bundle id (ej: com.miapp) o ruta a un .apk/.app: ',
  };
  ...
  try {
    ...
    let choice;
    for (;;) {
      choice = await ask('  Opción [1-8]: ');
      if (choice === '7') { rl.close(); return update(); }
      if (choice === '8' || choice === '') { rl.close(); return process.exit(0); }
      if (KEYS[choice]) break;
      log(s.warn('Opción inválida. Elegí un número del 1 al 8.'));
    }

    const target = await ask(ASKS[choice]);
    const extra = await askScope(choice, ask, target);
    rl.close();
...
async function askScope(choice, ask, target) {
  if (choice === '1' || choice === '2') return askCasos(ask);   // QA funcional / seguridad
  if (choice === '3') return askCarga(ask);                     // carga k6
  if (choice === '5') return askServicio(ask, target);          // API REST
  if (choice === '6') return askMovil(ask, target);             // app móvil
...
// Servicio (API REST). Dos modos:
//  - sin URL (target vacío): se prueba un ARCHIVO con varios endpoints.
//  - con URL: se configura un único endpoint acá (método/headers/delay/body).
async function askServicio(ask, target) {
  if (!target) return askServicioArchivo(ask);
  return askServicioUnico(ask);
}

// Modo archivo: lista de endpoints en curls / OpenAPI / Postman / CSV / texto.
async function askServicioArchivo(ask) {
  const ruta = await ask('  Ruta del archivo con los endpoints ' +
    '(curls / OpenAPI-Swagger / colección Postman / CSV / lista): ');
  if (!ruta) return 'No se indicó endpoint ni archivo: pedí al usuario la URL o el archivo en el informe y no inventes endpoints.';
  const existe = fileExists(ruta);
  const delay = await ask('  Delay entre requests en ms (Enter = 0): ');
  return [
    'Servicio/API a probar (REST) — VARIOS endpoints desde un archivo:',
    '- Archivo de endpoints: ' + ruta + (existe ? '' : '  (¡no se encontró en disco, verificá la ruta!)'),
    '- Leé ese archivo (puede ser curls, OpenAPI/Swagger, una colección Postman, CSV o una lista).',
    '- Si ya es una colección Postman, ejecutala directo con newman; si no, generá UNA colección que cubra TODOS los endpoints.',
    '- Generá casos para cada endpoint y reportá los resultados de todos juntos.',
    '- Delay entre requests: ' + (delay || '0') + ' ms',
  ].join('\n');
}

// Modo único: configuración de un endpoint puntual.
async function askServicioUnico(ask) {
  ...
  return [
    'Servicio/API a probar (REST) — un endpoint:',
    '- Método HTTP: ' + method,
...
// App móvil (Flutter/nativa) con Maestro: plataforma, binario y flujos a probar.
async function askMovil(ask, target) {
  log('\n' + s.hr('¿En qué plataforma?') + '\n');
  log(menuRow(1, 'Android ' + c.gray + '(emulador o dispositivo)' + c.reset));
  log(menuRow(2, 'iOS ' + c.gray + '(simulador · requiere Mac + Xcode)' + c.reset));
  log(menuRow(3, 'Ambas ' + c.gray + '(Android e iOS)' + c.reset) + '\n');
  const p = await ask('  Opción [1-3] (Enter = 1): ');
  const PLATS = { 2: 'iOS', 3: 'Android e iOS' };
  const plat = PLATS[p] || 'Android';

  const esApk = /\.(apk|app|ipa)$/i.test(target);
  const instalar = esApk
    ? 'Instalá la app desde el binario: ' + target
    : 'Asumí que la app (' + (target || 'appId no indicado') + ') ya está instalada en el emulador/dispositivo; si no, pedí el .apk/.app en el informe.';

  log('\n' + s.hr('¿Qué flujos querés probar?') + '\n');
  log(menuRow(1, 'Describir los flujos en lenguaje natural'));
  log(menuRow(2, 'Indicar un archivo de flujos/casos ' + c.gray + '(.yaml de Maestro, .md, .csv)' + c.reset));
  log(menuRow(3, 'Generar automáticamente ' + c.gray + '(explorar la app)' + c.reset) + '\n');
  const k = await ask('  Opción [1-3] (Enter = 3): ');

  let flujos;
  if (k === '1') {
    const txt = await ask('  Describí los flujos (ej: login, alta, navegación): ');
    flujos = txt ? 'Flujos a cubrir (lenguaje natural):\n' + txt : 'Generá los flujos explorando la app.';
  } else if (k === '2') {
    const ruta = await ask('  Ruta del archivo de flujos/casos: ');
    flujos = ruta ? 'Leé y usá como base los flujos del archivo: ' + ruta : 'Generá los flujos explorando la app.';
  } else {
    flujos = 'Generá los flujos automáticamente explorando la app (pantallas principales, login, navegación).';
  }

  return [
    'App móvil a probar (con Maestro):',
    '- Plataforma(s): ' + plat,
    '- App / binario: ' + (target || 'no indicado'),
    '- ' + instalar,
    '- ' + flujos,
  ].join('\n');
}

// ¿La ruta apunta a un archivo existente?
